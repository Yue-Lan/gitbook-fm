...
看到desktop window的realize和map都是重写的，这里采用的是XAtom的DESKTOP标志实现的desktop window属性。有了这些准备工作，一个桌面窗口就能显现在我们眼前了。

也许我们也可以直接从window继承来实现desktop window，这些其实都是自由的。这里的要点是我们从中学到了一些对窗口进行处理的技巧，而且事实上比想象中的要简单。
